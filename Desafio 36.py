cores = {    #cores normais

         'vermelho':'\033[31m',
         'azul':'\033[34m',
         'amarelo':'\033[33m',
         'branco':'\033[30m',
         'roxo':'\033[35m',
         'verde':'\033[32m',
         'ciano':'\033[36m',
         'limpa':'\033[m',
         'preto e branco':'\033[7;30;m',

                #cores em negrito

         'vermelho em negrito':'\033[1;31m',
         'azul em negrito':'\033[1;34m' ,
         'amarelo em negrito':'\033[1;33m' ,
         'branco em negrito':'\033[1;30m',
         'roxo em negrito':'\033[1;35m',
         'verde em negrito':'\033[1;32m',
         'ciano em negrito':'\033[1;36m',

               #cores sublinhadas

         'vermelho sublinhado':'\033[4;31m',
         'azul sublinhado':'\033[4;34m',
         'amarelo sublinhado':'\033[4;33m',
         'branco sublinhado':'\033[4;30m',
         'roxo sublinhado':'\033[4;35m',
         'verde sublinhado':'\033[4;32m',
         'ciano sublinhado':'\033[4;36m'
         }
print("\033[7;30mPrograma para fazer um empréstimo bancário para uma casa\033[m")
valorcasa = float(input("\033[1;35mInforme valor da casa?\033[m"))
salario = float(input("\033[1;35mInforme valor do salario?\033[m"))
anos = int(input("\033[1;35mEm quandos anos Deseja pagar\033[m:"))
prestaçao = (anos*12)
valor = valorcasa/prestaçao
salario= salario - (salario*30/100)
if salario >= valor:
    print("\033[7;30mEmprestimo aceito.\033[m")
    print("\033[7;30mSua prestação sera {} em {} anos\033[m".format(valor,anos), end=" ")
else:
    print("\033[7;30mEmprestimo Não aceito\033[m")
